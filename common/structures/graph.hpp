...
class Graph {
    ...
    std::expected<VerticesPair, GraphEdgeActionResult> try_get_edge_vertices(
        ...
        if (not A.has_value() and not B.has_value()) return std::unexpected(GraphEdgeActionResult::SrcAndDestUnknown);
        if (not A.has_value()) return std::unexpected(GraphEdgeActionResult::SrcUnknown);
        if (not B.has_value()) return std::unexpected(GraphEdgeActionResult::DestUnknown);
...
public:
    ...
    GraphEdgeActionResult remove_edge(const std::string &src, const std::string &dest) {
        ...
        if (it == edges_.end()) return GraphEdgeActionResult::EdgeUnknown;
    ...
    GraphVertexActionResult remove_vertex(const std::string &name) {
        ...
        if (not u.has_value()) return GraphVertexActionResult::UnknownVertex;
