...
class Dinic {
    using EdgeWeights = std::unordered_map<Edge*, std::pair<i64, i64>>;
    ...
    struct ResidualArc {
        ...
        [[nodiscard]] i64 get_capacity(EdgeWeights& res) const {
            return not reversed ? res[edge].first : res[edge].second;
        }

        void push(EdgeWeights& res, i64 x) const {
            if (not reversed) {
                res[edge].first -= x;
                res[edge].second += x;
            } else {
    ...
    static std::unordered_map<std::string, i64> group_vertices_to_levels_bfs(
        const Graph& graph, const std::string& start, EdgeWeights& edge_forward_and_reversed_weights) {
    ...
    static i64 try_dfs(const std::string& u,
                       const std::string& end,
                       std::unordered_map<std::string, i64>& levels,
                       const Graph& graph,
                       EdgeWeights& edge_forward_and_reversed_weights,
        ...
        while (std::cmp_less(next_arc_index[u], arcs.size())) {
            const auto& e = arcs[next_arc_index[u]];
            const auto& v = e.to();

            const i64 c = e.get_capacity(edge_forward_and_reversed_weights);
            if (c <= 0 or not levels.contains(v) or not levels.contains(u) or levels[v] - levels[u] != 1) {
                next_arc_index[u]++;
                continue;
            }

            const auto sent = try_dfs(v,
                                      end,
                                      levels,
                                      graph,
    ...
    static i64 get_blocking_flow_cost(const Graph& graph,
                                      std::unordered_map<std::string, i64>& levels,
                                      EdgeWeights& edge_forward_and_reversed_weights,
  ...
  public:
    static i64 max_flow(const Graph& graph, const std::string& start, const std::string& end) {
        ...
        while (true) {
            ...
            if (not levels.contains(end)) {
                break;
            }

            sum_flow += get_blocking_flow_cost(graph, levels, edge_forward_and_reversed_weights, start, end);
        }
...
void solve() {
    TaskRepl task_repl;
    task_repl.register_cmd("MAXFLOW", [](const Graph& g) {
